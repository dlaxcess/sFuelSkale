import { bytesToHex, encodePacked, hexToBigInt, hexToNumber, isAddress, isHex, keccak256, maxUint256, numberToHex } from "viem";
...
async function _mineFreeGas(gasAmount: number, address: `0x${string}`, nonce: number): Promise<{
  duration: number;
  gasPrice: bigint;
}> {
